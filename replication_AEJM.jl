...
function replicate(folder = "../Replication/"; loaddir = "../Output/", datadir = "../Data/")
    ...
    fig16_app = makeplots_minimal(sm_norep, move="d", slides = false)
    savefig(fig16_app, folder * "minimal_tfp_d_norep.pdf", width=900, height=300)
    ...
    fig9 = panels_crises_small(p_bench, 400, :spread, k = 1, k_back = 11, thres_back = 350, slides = false, get_stats = true, filename = folder * "/tab7.txt")
    ...
    # Figure 14: Outcomes and inequality (0.1 effect in text shown when running)
    ...
    fig15 = panels_IRF_cs(pIRF_bench1, pIRF_hi, pIRF_lo, cond_Y = 0.95, slides=false, τ_hi=sd_hi.pars[:τ], τ_lo=sd_lo.pars[:τ], give_stats=true)
    savefig(fig15, folder * "/panels_IRF_cs.pdf", width = 900, height = 500)

    # Figure 17: Value functions in the crisis
    fig17_app = distribution_IRF(pIRF_bench, pIRF_nodef, pIRF_samep, cond_Y = 0.95, slides = false)
    savefig(fig17_app, folder * "distribIRF_paper.pdf", width = 800, height = 400)

    # Figure 18: Subjective probabilities of default
    fig18 = make_twisted(sd_bench, slides = false, leg = true)
    savefig(fig18, folder * "twistedprobs_paper.pdf", width = 800, height = 350)

    # Figures 19 and 20 are above

    # Figure 21: Market Clearing in the Nontradable Sector
