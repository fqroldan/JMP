...
g, p_nob, _, _, _ = make_simulated_path(sd3, "../Output/run3/", 10_000, K = 50);
Wr = mean([mean(series(p, :Wr)) for p in p_nob])
FileIO.save("../Output/SOEdef_nob.jld2", "sd", sd3, "g", g, "pp", p_nob, "Wr", Wr)
...
# end
# update_probs!(sd3)
# mpe_iter!(sd3, run_number = 3, nodef = false, noΔ = true, maxiter = 500, tol = 25e-4)
